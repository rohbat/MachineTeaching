from flask import Flask, jsonify, request, render_template, redirect, url_for, session
# from sqlalchemy.orm.session import sessionmaker, make_transient
# from sqlalchemy import create_engine
# from database import db
from page_model_no_db import PageModel
import glob
import os 
import random
import numpy as np
from cython_tste.tste_next_point import *
import time
import hashlib
import sys

print 'Initializing site!'

tern1 = [531, 532, 533, 534, 535, 536, 537, 538, 539, 540, 541, \
542, 543, 544, 545, 546, 547, 548, 549, 550, 551, 552, \
553, 554, 555, 556, 557, 558, 559, 560, 561, 562, 563, 564, \
565, 566, 567, 568, 569, 570, 571, 572, 573, 574, 575, 576, \
577, 578, 579, 580, 581, 582, 583, 584, 585, 586, 587, 588, 589, 590]
tern2 = [473, 474, 475, 476, 477, 478, 479, 480, 481, 482, 483, 484, \
485, 486, 487, 488, 489, 490, 491, 492, 493, 494, 495, 496, 497, 498, \
499, 500, 501, 502, 503, 504, 505, 506, 507, 508, 509, 510, 511, 512, \
513, 514, 515, 516, 517, 518, 519, 520, 521, 522, 523, 524, 525, 526, \
527, 528, 529, 530]
black1 = [60, 61, 62, 63, 64, 65, 66, 67, 68, 69, 70, 71, 72, 73, 74, \
75, 76, 77, 78, 79, 80, 81, 82, 83, 84, 85, 86, 87, 88, 89, 90, 91, 92,\
 93, 94, 95, 96, 97, 98, 99, 100, 101, 102, 103, 104, 105, 106, 107, 108, \
 109, 110, 111, 112, 113, 114, 115, 116, 117, 118, 119]
black2 = [120, 121, 122, 123, 124, 125, 126, 127, 128, 129, 130, \
131, 132, 133, 134, 135, 136, 137, 138, 139, 140, 141, 142, 143, 144, \
145, 146, 147, 148, 149, 150, 151, 152, 153, 154, 155, 156, 157, 158, 159, \
160, 161, 162, 163, 164, 165, 166, 167, 168, 169, 170, 171, 172, 173, 174, \
175, 176, 177, 178]
brown1 = [413, 414, 415, 416, 417, 418, 419, 420, 421, 422, 423, 424, 425, 426, 427, 428, 429, 430, 431, 432, 433, 434, 435, 436, 437, 438, 439, 440, 441, 442, 443, 444, 445, 446, 447, 448, 449, 450, 451, 452, 453, 454, 455, 456, 457, 458, 459, 460, 461, 462, 463, 464, 465, 466, 467, 468, 469, 470, 471, 472]
brown2 = [353, 354, 355, 356, 357, 358, 359, 360, 361, 362, 363, 364, 365, 366, 367, 368, 369, 370, 371, 372, 373, 374, 375, 376, 377, 378, 379, 380, 381, 382, 383, 384, 385, 386, 387, 388, 389, 390, 391, 392, 393, 394, 395, 396, 397, 398, 399, 400, 401, 402, 403, 404, 405, 406, 407, 408, 409, 410, 411, 412]

def get_hard(n): 
    hard = []
    temp = np.random.randint(6, size=n)
    for i in range(n): 
        if temp[i] == 0: 
            i = random.choice(tern1)
            j = random.choice(tern1)
            k = random.choice(tern2)
        elif temp[i] == 1: 
            i = random.choice(tern2)
            j = random.choice(tern2)
            k = random.choice(tern1)
        elif temp[i] == 2: 
            i = random.choice(black1)
            j = random.choice(black1)
            k = random.choice(black2)
        elif temp[i] == 3: 
            i = random.choice(black2)
            j = random.choice(black2)
            k = random.choice(black1)
        elif temp[i] == 4: 
            i = random.choice(brown1)
            j = random.choice(brown1)
            k = random.choice(brown2)
        elif temp[i] == 5: 
            i = random.choice(brown2)
            j = random.choice(brown2)
            k = random.choice(brown1)


        if random.random() > .5:
            hard.append((i, j, k))
        else: 
            hard.append((i, k, j))

    return hard







def update_page_with_indices(main, comp1, comp2):
    # Switch order of images with probability .5
    if random.random() > .5:
        temp = comp1
        comp1 = comp2
        comp2 = temp

    page_model_dict_bird[session['name']].main_img = main
    page_model_dict_bird[session['name']].compare_img_1 = comp1
    page_model_dict_bird[session['name']].compare_img_2 = comp2

    page_model_dict_bird[session['name']].main_path = image_list[main]
    page_model_dict_bird[session['name']].compare_1_path = image_list[comp1]
    page_model_dict_bird[session['name']].compare_2_path = image_list[comp2]



# Choose a random triplet and set the page's triplet accordingly

def update_page(selection_method):
    train = range(N)
    X = user_x_dict_bird[session['name']]
    lamb = 0

    if selection_method == 1:
        (main, comp1, comp2) = random_triplet(train, classes, classes_dict_bird)
    elif selection_method == 2:
        ((main, comp1, comp2), p) = most_uncertain_triplet(train,X,N,no_dims,alpha,lamb,classes,classes_dict_bird,eta,no_classes=3,sample_class = 0.135)
    elif selection_method == 3:
        ((main, comp1, comp2), p) = best_gradient_triplet(train,X,N,no_dims,alpha,lamb,classes,classes_dict_bird,eta,no_classes=3,sample_class = 0.06)
    else:
        ((main, comp1, comp2), p) = best_gradient_triplet_rand_evaluation(train,X,N,no_dims,alpha,lamb,classes,classes_dict_bird,eta,no_classes=3,sample_class = 0.025)

    update_page_with_indices(main, comp1, comp2)


def update_test(): 
    (main, comp1, comp2) = user_test_images_dict_bird[session['name']][user_test_counter_dict_bird[session['name']]-1]
    # print user_test_counter_dict_bird[session['name']], (main, comp1, comp2)
    update_page_with_indices(main, comp1, comp2)



def get_label_list():
    main_label = class_names[classes[page_model_dict_bird[session['name']].main_img]]
    compare_img_1_label = class_names[classes[page_model_dict_bird[session['name']].compare_img_1]]
    compare_img_2_label = class_names[classes[page_model_dict_bird[session['name']].compare_img_2]]
    return (main_label, compare_img_1_label, compare_img_2_label)


def get_result_img(result):
    if result == True:
        return '/static/correct_check_mark.png'
    else:
        return '/static/incorrect_x_mark.png'

def get_result_text(result): 
    if result == True: 
        return 'CORRECT! '
    else: 
        return 'INCORRECT! '

def get_result_color(result): 
    if result == True: 
        return '#1E90FF'
    else: 
        return 'red'

def get_test_images(selection_method): 
    result = []
    train = range(N)

    for i in range(N_test): 
        result.append(random_triplet(train, classes, classes_dict_bird))
    
    temp = get_hard(diff_test)
    print 'temp', temp

    
    result = result + temp
    print 'result', result
    return result


# Make Flask app

app = Flask(__name__)
#app.config["DEBUG"] = True




# # Set up database

# SQLALCHEMY_DATABASE_URI = "mysql+mysqlconnector://{username}:{password}@{hostname}/{databasename}".format(
#     username="cs101teaching2",
#     password="gogo_teaching",
#     hostname="cs101teaching2.mysql.pythonanywhere-services.com",
#     databasename="cs101teaching2$teaching",
# )
# app.config["SQLALCHEMY_DATABASE_URI"] = SQLALCHEMY_DATABASE_URI
# app.config["SQLALCHEMY_POOL_RECYCLE"] = 200
app.secret_key = 'A0Zr98j/3yX R~XHH!jmN]LWX/,?RT'

# db.init_app(app)
# db.app = app

# engine = create_engine(SQLALCHEMY_DATABASE_URI, pool_recycle=200)

# Session_sql = sessionmaker(bind=engine, expire_on_commit=False)
# session_sql = Session_sql()



# Set up classes and classes_dict_bird
import os
path = os.getcwd()
class_names = glob.glob(path + "/MachineTeaching/static/birds/images/*")
class_names.sort()

class_name_dict_bird = {}
for class_name in class_names:
    class_name_dict_bird[class_name] = glob.glob(class_name + "/*")

name_class = {}
for k, v in class_name_dict_bird.iteritems():
    for elm in v:
        name_class[elm] = k

image_list = glob.glob(path + "/MachineTeaching/static/birds/images/*/*")
image_list.sort()
N = len(image_list)

classes = np.zeros(len(image_list), dtype=int)
for i in range(len(image_list)):
    classes[i] = class_names.index(name_class[image_list[i]])


class_names = [c.replace(path + "/MachineTeaching/static/birds/images/", "") for c in class_names]
print 'class names: ', class_names

classes_dict_bird = {}
for i in range(len(class_names)):
    classes_dict_bird[i] = []

N_test = 20

for i in range(N):
    classes_dict_bird[classes[i]].append(i)

for key in classes_dict_bird.keys():
    classes_dict_bird['not'+str(key)] = []
    for key1 in classes_dict_bird.keys():
        if key != key1 and not 'not' in str(key1):
            classes_dict_bird['not'+str(key)].extend(classes_dict_bird[key1])

# print classes, len(classes)
# print classes_dict_bird, len(classes_dict_bird)



# Set values for tste

N = len(classes)
no_dims = 5
alpha = no_dims - 1.0
eta = 0.1



# Make image list

image_list = [img.replace(path + "/MachineTeaching", "") for img in image_list]



# Make dictionary to keep track of page models
page_model_dict_bird = {}



# Make dictionary to keep track of how many jobs each user has done
user_nclicks_dict_bird = {}
user_x_dict_bird = {}
user_id_dict_bird = {}
user_time_dict_bird = {}
user_code_dict_bird = {}
user_test_counter_dict_bird = {}
user_selection_method_dict_bird = {}
user_images_dict_bird = {}
user_test_images_dict_bird = {}
user_test_error_dict_bird = {}
user_test_ans_dict_bird = {}
user_test_time_dict_bird = {}
user_train_ans_dict_bird = {}

# Set triplet

random.seed()
counter = 0
max_clicks = 30
diff_test = 10
max_test = N_test + diff_test


# print 'difficult_all', difficult_all

# Redirect user to login page

@app.route("/")
def to_login():
    return redirect(url_for('route'))

# Return image list in JSON format 

@app.route("/get_imgs", methods = ['GET', 'POST'])
def get_imgs():
    # print 'nclicks', user_nclicks_dict_bird
    if user_nclicks_dict_bird[session['name']] == max_clicks:
        # user_test_images_dict_bird[session['name']] = random.sample(set(range(N)) - user_images_dict_bird[session['name']], N_test)

        user_test_images_dict_bird[session['name']] = get_test_images('random')
        user_test_counter_dict_bird[session['name']] = 1
        user_test_error_dict_bird[session['name']] = 0
        user_test_ans_dict_bird[session['name']] = []
        user_test_time_dict_bird[session['name']] = time.time()
        # return jsonify([url_for('testing_index'), 0])
        return jsonify([url_for('to_test'), 0])

    update_page(user_selection_method_dict_bird[session['name']])
    user_images_dict_bird[session['name']].update(page_model_dict_bird[session['name']].get_index_list())


    return jsonify(page_model_dict_bird[session['name']].get_imgs_list() + [str(user_nclicks_dict_bird[session['name']]), get_label_list()[0]]) 


# # Test stuff
# @app.route("/get_test_img", methods = ['GET', 'POST'])
# def get_test_img():
#     if user_test_counter_dict_bird[session['name']] == max_test: 
#         return jsonify([url_for('logout'), 0])
#     # user_test_counter_dict_bird[session['name']] += 1
#     return jsonify([image_list[user_test_images_dict_bird[session['name']]\
#         [user_test_counter_dict_bird[session['name']]-1]], \
#         str(user_test_counter_dict_bird[session['name']]), 0]) 


@app.route("/testing/get_imgs", methods = ['GET', 'POST'])
def testing_get_imgs():
    if user_test_counter_dict_bird[session['name']] == max_test: 
        return jsonify([url_for('logout'), 0])

   
    update_test() 
    return jsonify(page_model_dict_bird[session['name']].get_imgs_list() \
        + [str(user_test_counter_dict_bird[session['name']])]) 



# THESE DO LOTS
#
# - Gets the response from the user
# - Updates the user's kernel

@app.route("/kernel/get_response", methods = ['POST'])
def get_response_kernel():
    if not 'name' in session or not session['name'] in user_nclicks_dict_bird:
        return jsonify([url_for('login'), 0])
    if request.method == 'POST':
        data = request.get_data()
        (main_label, c1_label, c2_label) = get_label_list()
        if data == "0":
            page_model_dict_bird[session['name']].set_chosen(page_model_dict_bird[session['name']].compare_img_1)
            if (main_label == c1_label):
                result = True
            else:
                result = False
        elif data == "1":
            page_model_dict_bird[session['name']].set_chosen(page_model_dict_bird[session['name']].compare_img_2)
            if (main_label == c2_label):
                result = True
            else:
                result = False
        # print 'RESULT: ', result
        user_train_ans_dict_bird[session['name']].append(result)
        # print 'TRAIN ANS: ', user_train_ans_dict_bird[session['name']]
        user_nclicks_dict_bird[session['name']] += 1
        user_time_dict_bird[session['name']][1] = time.time()
        K = np.zeros((N, N))
        Q = np.zeros((N, N))
        G = np.zeros((N, no_dims))
        tste_grad(user_x_dict_bird[session['name']], N, no_dims, page_model_dict_bird[session['name']].get_index_list(), 0, no_dims-1.0, K, Q, G)
        user_x_dict_bird[session['name']] = user_x_dict_bird[session['name']] - 0.4 * G
        #print user_x_dict_bird[session['name']]

    # make_transient(page_model_dict_bird[session['name']])
    # page_model_dict_bird[session['name']].id = None
    # session_sql.add(page_model_dict_bird[session['name']])
    # session_sql.commit()

    if main_label == c1_label: 
        c1_border = '5px solid #1E90FF'
        c2_border = '0px solid #1E90FF'
    elif main_label == c2_label: 
        c1_border = '0px solid #1E90FF'
        c2_border = '5px solid #1E90FF'

    imgs = page_model_dict_bird[session['name']].get_imgs_list()
    plt.figure()
    plt.title('Method ' + str(user_selection_method_dict_bird[session['name']]) + ': '\
     str(user_nclicks_dict_bird[session['name']]) + ' | (' + main_label + ', ' + c1_label + ', '\
        + c2_label + ')')
    for i in range(3): 
        im = mpimg.imread(imgs[i])
        plt.imshow(img)
        plt.axis('off')
    plt.savefig('./imgs/' + str(user_selection_method_dict_bird[session['name']]) + '_' + \
        str(user_nclicks_dict_bird[session['name']]))

    
    return jsonify([main_label, c1_label, c2_label, get_result_text(result), get_result_color(result), c1_border, c2_border])
    # return jsonify([main_label, c1_label, c2_label, get_result_img(result)])


@app.route("/testing/get_response", methods = ['POST'])
def get_response_testing():
    if request.method == 'POST':
        # print 'clicked'
        data = request.get_data()
        (main_label, c1_label, c2_label) = get_label_list()

        if data == '0': 
            if main_label == c1_label: 
                result = True
            else:
                result = False
        elif data == '1': 
            if main_label == c2_label: 
                result = True
            else: 
                result = False

        # print 'result: ', result

        user_test_ans_dict_bird[session['name']].append(result)

    if user_test_counter_dict_bird[session['name']] == max_test: 
        return jsonify([url_for('logout'), 0])

    user_test_counter_dict_bird[session['name']] += 1
    update_test() 
    
    return jsonify(page_model_dict_bird[session['name']].get_imgs_list()  +\
     [str(user_test_counter_dict_bird[session['name']])]) 



# Render main page
@app.route("/teaching/")
def teaching_index():
    if not 'name' in session or not session['name'] in user_nclicks_dict_bird:
        return redirect(url_for('login'))
    return render_template('teaching_triplet_chinese.html')

@app.route("/to_testing", methods=['GET', 'POST'])
def to_test(): 
    if not 'name' in session or not session['name'] in user_test_counter_dict_bird:
        return redirect(url_for('login'))
    elif request.method == 'POST' and request.form['cont'] == "Continue":
        return redirect(url_for('testing_index'))
    return render_template('begin_testing.html')

@app.route("/testing/")
def testing_index():
    if not 'name' in session or not session['name'] in user_test_counter_dict_bird:
        return redirect(url_for('login'))
    return render_template('test_triplet_chinese.html')


counter = 0
# Create new user
#
# - Store given user name
# - Create initial kernel for new user

urls = ['http://cs101teaching2.pythonanywhere.com/login', 
'http://machineteaching1.pythonanywhere.com/login', 
'http://machineteaching2.pythonanywhere.com/login', 
'http://machineteaching3.pythonanywhere.com/login', 
'http://machineteaching4.pythonanywhere.com/login', 
'http://machineteaching5.pythonanywhere.com/login']

@app.route('/login', methods=['GET', 'POST'])
def login():
    global counter
    error = None
    if request.method == 'GET':
        session['name'] = str(urls.index(request.url)) + "," + str(counter)
        print "Session name", session['name']
        counter += 1
    if request.method == 'POST' and request.form['cont'] == "Continue":
        np.save('./testfiles_bird/nclicks_dict_bird.npy', user_nclicks_dict_bird)
        np.save('./testfiles_bird/time_dict_bird.npy', user_time_dict_bird)
        np.save('./testfiles_bird/code_dict_bird.npy', user_code_dict_bird)
        page_model_dict_bird[session['name']] = PageModel()
        user_selection_method_dict_bird[session['name']] = random.randint(1, 4)
        user_nclicks_dict_bird[session['name']] = 0
        user_time_dict_bird[session['name']] = [time.time(), 0]
        user_train_ans_dict_bird[session['name']] = []
        user_x_dict_bird[session['name']] = np.load("MachineTeaching/static/X_initial_bird.npy")
        #user_x_dict_bird[session['name']] = np.random.rand(N, no_dims)
        user_images_dict_bird[session['name']] = set([])

        # print 'Selection Method: ' + str(user_selection_method_dict_bird[session['name']])

        return redirect(url_for('teaching_index'))
    return render_template('login_instr_triplet.html', error=error)



@app.route("/end/")
def logout():
    if ('name' in session and session['name'] in user_nclicks_dict_bird and 
        user_nclicks_dict_bird[session['name']] == max_clicks and user_test_counter_dict_bird[session['name']] == max_test):
            
            user_test_time_dict_bird[session['name']] = time.time() - user_test_time_dict_bird[session['name']] 
            user_test_error_dict_bird[session['name']] = 1-float(np.sum(user_test_ans_dict_bird[session['name']])) / float(max_test)
            end_id = hashlib.md5(str(session['name'])).hexdigest()

            # print 'name: ', session['name'], '\n'
            # print 'method: ', user_selection_method_dict_bird[session['name']], '\n'
            # print 'error: ', user_test_error_dict_bird[session['name']], '\n'

            np.save('./testfiles_bird/user_x_dict_bird.npy', user_x_dict_bird)
            np.save('./testfiles_bird/ans_dict_bird.npy', user_test_ans_dict_bird)
            np.save('./testfiles_bird/test_images_dict_bird.npy', user_test_images_dict_bird)
            np.save('./testfiles_bird/error_dict_bird.npy', user_test_error_dict_bird)
            np.save('./testfiles_bird/method_dict_bird.npy', user_selection_method_dict_bird)
            np.save('./testfiles_bird/train_ans_dict_bird.npy', user_train_ans_dict_bird)
            np.save('./testfiles_bird/test_time_dict_bird.npy', user_test_time_dict_bird)
            with open('./testfiles_bird/' + str(session['name']) + "_test.txt", "w") as myfile:
                for item in user_test_ans_dict_bird[session['name']]: 
                    myfile.write("%s," % item)
                myfile.write("\n")
                myfile.write(str(user_test_error_dict_bird[session['name']]) + '\n')
                myfile.write(str(user_test_time_dict_bird[session['name']]) + '\n')
                myfile.write(str(user_selection_method_dict_bird[session['name']]) + '\n')

            return render_template('end.html', end_id=end_id)
    else:
        return redirect(url_for('login'))



routing_counter = 0
@app.route('/route', methods=['GET', 'POST'])
def route():
    global routing_counter
    routing_counter += 1
    return redirect(urls[routing_counter%len(urls)])


# Run
if __name__ == "__main__":
    app.run()
