...
def teach_response_rand(i, j, k, x): 
    r = np.random.uniform(0., 1.)
    if r < 0.5: 
        return [i, j, k]
    else: 
...
def teach_response_maj(i, j, k, X): 
    diff = X[i, :] - X[j, :]
    k_ij = np.dot(diff, diff)
    k_ij = (1 + k_ij / alpha) ** ((alpha + 1) / -2)
    ...
    for i in range(n): 
        for key in range(4): 
            for j in range(num_user): 
                ...
                X_dict[key][j] = X - 0.4 * G
                # print(key, np.sum(G))
                triplet_dict[key][j, n*rounds + i] = np.asarray(chosen)
    ...
    for i in range(n): 
        for key in range(4): 
            for j in range(num_user): 
                ...
                acc_dict[key][j, n*rounds + i] = np.asarray(chosen)
...
def main(): 
    ...
    for i in range(4): 
        triplet_dict[i] = np.zeros([num_user, n_teach * rounds, 3])
        test_dict[i] = np.zeros([num_user, n_test * rounds, 3])
        # X_dict[i] = np.zeros([num_user, np.shape(X)[0], np.shape(X)[1]])
        # for j in range(num_user): 
        #     X_dict[i][j] = X
    X_dict = np.load('\\Users\\Audrey\\Documents\\cs101a\\sim\\X_dict_perf_5.npy').item()
